...
static void wakeup_mpu6050()
{
    uint8_t buf[] = {0x6B, 0x00};
    i2c_write_blocking(i2c_default, addr, buf, 2, false);
}

static uint8_t mpu6050_who_I_am()
{
    uint8_t val = 0x75;
    uint8_t my_buf = 0x00;
    i2c_write_blocking(i2c_default, addr, &val, 1, true);
...
static void set_mpu6050_acc_config()
{
    uint8_t buf[] = {0x1C, 0x00};
    i2c_write_blocking(i2c_default, addr, buf, 2, false);
}
static void set_mpu6050_gyro_config()
{
    uint8_t buf[] = {0x1B, 0x18};
    i2c_write_blocking(i2c_default, addr, buf, 2, false);
}
static void read_mpu6050_acc_config()
{
    uint8_t val = 0x1C;
    uint8_t my_buf = 0xFF;
    i2c_write_blocking(i2c_default, addr, &val, 1, true);
...
static void read_mpu6050_gyro_config()
{
    uint8_t val = 0x1B;
    uint8_t my_buf = 0xFF;
    i2c_write_blocking(i2c_default, addr, &val, 1, true);
...
static void calculate_attitude(int16_t accel[3], int16_t gyro[3], float acc[3], float _gyro[3], float pitch, float yaw, float roll)
{
    for(int i = 0; i < 3; ++i)
    {
