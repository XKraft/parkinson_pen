...
void get_time_ms(uint32_t* count)
{
...
uint8_t MPU6050_DMP_Init()
{
	...
	if(!mpu_init(p))                                                               //mpu初始化
	{	
		res = mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL);//设置需要的传感器
        if(res)
        {
            printf("mpu init error 1\n");
            return 1;
        }
		res = mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);//设置fifo
        if(res)
        {
            printf("mpu init error 2\n");
            return 2;
        }
		res = mpu_set_sample_rate(DEFAULT_MPU_HZ);//设置采集样率,多久更新fifo，并产生int中断
        if(res)
        {
            printf("mpu init error 3\n");
            return 3;
        }
		res = dmp_load_motion_driver_firmware();//加载dmp固件
        if(res)
        {
            printf("mpu init error 4, res = %d\n", res);
            return 4;
        }
		res = dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//设置陀螺仪方向
        if(res)
        {
            printf("mpu init error 5\n");
            return 5;
        }
        ...
        if(res)
        {
            printf("mpu init error 6\n");
            return 6;
        }
		res = dmp_set_fifo_rate(DEFAULT_MPU_HZ);//设置采集样率,多久更新fifo，并产生int中断
        if(res)
        {
            printf("mpu init error 7\n");
            return 7;
        }
		// run_self_test();//自检
